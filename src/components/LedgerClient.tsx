    ...
    <div className="h-full flex flex-col bg-slate-100/50 overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden relative">
        {viewMode === 'table' ? (
          ...
          >
            <table className="ledger-table border-separate border-spacing-0 w-full">
              <thead className="sticky top-0 z-40">
                <tr className="bg-white">
                  <th className="ledger-header sticky-left z-50 border-b-0 py-1"></th>
                  <th colSpan={1 + allMethods.length + 1} className="ledger-header text-center font-black text-[10px] md:text-sm bg-red-50/80 text-red-700 tracking-[0.2em] border-b-0 divider-strong py-1.5 shadow-sm">
                    EXPENSES
                  </th>
                  <th colSpan={1 + allMethods.length + 1} className="ledger-header text-center font-black text-[10px] md:text-sm bg-green-50/80 text-green-700 tracking-[0.2em] border-b-0 py-1.5 shadow-sm">
                    CASH IN & BALANCE
                  </th>
                </tr>
                <tr>
                  <th className="ledger-header col-day sticky-left z-50 text-center border-t-0 pt-1 shadow-sm">Day</th>
                  <th className="ledger-header col-details bg-red-50/30">Expense Details</th>
                  {allMethods.map(m => (
                    <th key={`ex-h-${m}`} className="ledger-header col-amount text-right bg-red-50/30">{m}</th>
                  ))}
                  <th className="ledger-header col-total text-right bg-red-50/50 divider-strong">Total</th>
                  <th className="ledger-header col-details bg-green-50/30">Income Details</th>
                  {allMethods.map(m => (
                    <th key={`in-h-${m}`} className="ledger-header col-amount text-right bg-green-50/30">{m}</th>
                  ))}
                  <th className="ledger-header col-total text-right bg-green-50/50">Balance</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r) => (
                  <tr key={r.index} className={`ledger-row ${r.colorGroup} group`}>
                    {r.isFirst ? (
                      <td rowSpan={r.rowCount} className="ledger-cell text-center align-middle sticky-left font-bold text-fintech-text-main border-r border-fintech-border shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] bg-[#F8FAFC]">
                        {r.day}
                        <div className="text-[10px] font-normal text-fintech-text-muted uppercase">{r.dayName}</div>
                      </td>
                    ) : (
                    ...
                    <td
                      className="ledger-cell col-details truncate cursor-pointer transition-colors bg-red-50/10"
                    ...
                    {allMethods.map(m => (
                      <td key={`ex-${r.index}-${m}`} className="ledger-cell col-amount text-right tabular-nums text-fintech-text-muted bg-red-50/10">
                        {r.exp?.payment_method === m ? r.exp.amount.toLocaleString() : ''}
                      </td>
                    ))}
                    {r.isFirst ? (
                      <td rowSpan={r.rowCount} className="ledger-cell col-total text-right tabular-nums font-bold text-fintech-expense-text bg-red-50/30 align-middle divider-strong">
                        {r.dailyExpenseTotal > 0 ? r.dailyExpenseTotal.toLocaleString() : ''}
                      </td>
                    ) : (
                      <td className="hidden"></td>
                    )}
                    ...
                    <td
                      className="ledger-cell col-details truncate cursor-pointer transition-colors bg-green-50/10"
                    ...
                    {allMethods.map(m => (
                      <td key={`in-${r.index}-${m}`} className="ledger-cell col-amount text-right tabular-nums text-fintech-text-muted bg-green-50/10">
                        {r.inc?.payment_method === m ? r.inc.amount.toLocaleString() : ''}
                      </td>
                    ))}
                    {r.isFirst ? (
                      <td rowSpan={r.rowCount} className={`ledger-cell col-total text-right tabular-nums font-bold bg-green-50/30 align-middle ${r.dayEndBalance < 0 ? 'text-red-600' : 'text-fintech-primary'}`}>
                  ...
                  <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-100">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-fintech-text-muted">{first.dateStr}</div>
                      </div>
                    </div>
